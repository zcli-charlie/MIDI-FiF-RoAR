    ...
    def iteration(self, training_data, max_seq_len, train=True):
        ...
        for ori_seq_batch in pbar:
            ...
            for b in range(batch):
                ...
                for i in mask80:
                    input_ids[b][i] = self.midi_bert.mask_word
                    loss_mask[b][i] = 1 
                for i in rand10:
                    input_ids[b][i] = random.choice(range(len(self.midi_bert.e2w)))
                    loss_mask[b][i] = 1 
                for i in cur10:
                    loss_mask[b][i] = 1 
            ...
            attn_mask = (ori_seq_batch != self.midi_bert.pad_word).float().to(self.device)   # (4,512)
            ...
            # acc
            print('Loss: {:06f} | acc: {:03f} \r'.format(
                loss, acc), flush=True)
