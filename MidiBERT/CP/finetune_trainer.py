...
class FinetuneTrainer:
    def __init__(self, midi_bert, train_dataloader, valid_dataloader, test_dataloader, layer,
        ...
        if model != None:    # load model
            ...
            if SeqClass:
                self.model = SequenceClassification(self.midi_bert, class_num, hs).to(self.device)
            else:
                self.model = TokenClassification(self.midi_bert, class_num, hs).to(self.device)

#        for name, param in self.model.named_parameters():
#            if 'midi_bert.bert' in name:
