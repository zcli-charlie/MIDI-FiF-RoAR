...
class TokenClassification(nn.Module):
    def __init__(self, midi_bert, class_num, hs):
        super().__init__()
    ...
    def forward(self, y, attn, layer):
        # feed to bert 
        y = self.midi_bert(y, attn, output_hidden_states=True)
...
class SequenceClassification(nn.Module):
    def __init__(self, midi_bert, class_num, hs, da=128, r=4):
    ...
    def forward(self, x, attn, layer):             # x: (batch, 512, 4)
        x = self.midi_bert(x, attn, output_hidden_states=True)   # (batch, 512, 768)
        #y = y.last_hidden_state         # (batch_size, seq_len, 768)
