...
class BERTTrainer:
    def __init__(self, midi_bert: MidiBert, train_dataloader, valid_dataloader,
                lr, batch, max_seq_len, mask_percent, cpu, cuda_devices=None):
        self.device = torch.device("cuda" if torch.cuda.is_available() and not cpu else 'cpu')
        self.midi_bert = midi_bert        # save this for ckpt
        self.model = MidiBertLM(midi_bert).to(self.device)
    ...
    def iteration(self, training_data, max_seq_len, train=True):
        ...
        total_acc, total_losses = [0]*len(self.midi_bert.e2w), 0
        ...
        for ori_seq_batch in pbar:
            ...
            for b in range(batch):
                ...
                for i in mask80:
                    mask_word = torch.tensor(self.midi_bert.mask_word_np).to(self.device)
                    input_ids[b][i] = mask_word 
                    loss_mask[b][i] = 1 
                for i in rand10:
                    rand_word = torch.tensor(self.midi_bert.get_rand_tok()).to(self.device)
            ...
            attn_mask = (input_ids[:, :, 0] != self.midi_bert.bar_pad_word).float().to(self.device)   # (batch, seq_len)
            
            y = self.model.forward(input_ids, attn_mask)

            # get the most likely choice with max
            outputs = []
            for i, etype in enumerate(self.midi_bert.e2w):
            ...
            losses, n_tok = [], []
            for i, etype in enumerate(self.midi_bert.e2w):
                n_tok.append(len(self.midi_bert.e2w[etype]))
            ...
            print('Loss: {:06f} | loss: {:03f}, {:03f}, {:03f}, {:03f} | acc: {:03f}, {:03f}, {:03f}, {:03f} \r'.format(
                total_loss, *losses, *accs), flush=True) 
    ...
    def save_checkpoint(self, epoch, best_acc, valid_acc, 
        ...
        state = {
            'epoch': epoch + 1,
            'state_dict': self.midi_bert.state_dict(),
            'best_acc': best_acc,
            'valid_acc': valid_acc,
